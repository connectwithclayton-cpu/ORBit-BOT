...
_OUT_LOCAL = _BASE / "Fabio_debug_board.html"
...
def _git_rev() -> str:
    try:
        r = subprocess.run(
            ["git", "-C", str(_BASE), "rev-parse", "--short", "HEAD"],
            capture_output=True,
            text=True,
            timeout=3,
        )
...
def _collect_payload() -> dict:
    ...
    log_tail: list[str] = []
    log_path = _BASE / "orb_bot_fabio.log"
    if log_path.exists():
        try:
    ...
    return {
        ...
        "log_tail": log_tail,
        "cli": {
            "research_backtest": "python3 Fabio_orb_backtest.py",
            "live_mirror_backtest": "python3 Fabio_live_mirror_backtest.py",
            "verify_trades": "python3 verify_trades.py",
            "force_close": "python3 force_close.py",
        },
    }
