...
def _repo_base(start: Path | None = None) -> Path:
    return Path(__file__).resolve().parent if start is None else start.resolve()
...
def load_beta_manifest_records(base: Path | None = None) -> list[dict[str, Any]]:
    ...
    path = _repo_base(base) / _MANIFEST_NAME
    if not path.is_file():
        return []
    try:
