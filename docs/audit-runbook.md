...
3. **Verify** with full-stack audit when investigating “missing today’s positions”:
   ...
   ```
4. **Intraday** rolling check (market window): keep `scripts/audit_moomoo_sync.py` on schedule per below.
...
## Files
- Runner: `scripts/audit_moomoo_sync.py`
- Full stack: `audit_full_positions.py` (entire SIMULATE history vs Sheets + dashboard)
- Canonical publish gate: `verify_canonical_publish.py`
- Wrapper: `scripts/run_moomoo_sync_audit.sh`
- Scheduler installer: `install_sync_audit_scheduler.sh`
- Summary: `scripts/summarize_audit_sync.sh`
- Log artifact: `audit_sync.jsonl`
- State file: `audit_sync_state.json`

## Standard Commands
From `~/Documents/TRADING/Fabio_bot`:
...
- Manual fallback:
  - Run `scripts/run_moomoo_sync_audit.sh` directly.
